...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($errorsCounter == 0) {
        require_once ('../db.php');
        $sql = "update tb_references
            set testimonial = :testimonial, image = :image, person= :person, position= :position where id = :id;";
        ...
        $result->execute();
        $_SESSION['flash'] = ['status' => 'success', 'message' => 'Ссылка '.$_POST['Testimonial'].' успешно обновлена'];
        header("Location: /admin/references/");
    }
}
else {
    require_once ('../db.php');
    $sql = "select * from tb_references where id = :id;";
...
<?= breadcrumbs(
    [
        ...
        [
            'href' => '/admin/references/update.php',
            'title' => 'Update'
        ]
    ]
)?>
        ...
        <div class="row justify-content-center">
            <div class="col-4">
                <div class="mb-3">
                    ...
                    <label for="testimonial">Testimonial: </label><input type="text" class="form-control" name="Testimonial" id="testimonial" aria-describedby="emailHelp" value="<?=$data['testimonial'] ?? $_POST['Testimonial']?>">
                ...
                <div class="mb-3">
                    <label for="image">Image: </label><input type="text" class="form-control" name="Image" id="image" aria-describedby="emailHelp" value="<?=$data['image'] ?? $_POST['Image']?>">
                ...
                <div class="mb-3">
                    <label for="person">Person: </label><input type="text" class="form-control" name="Person" id="company" aria-describedby="emailHelp" value="<?=$data['person'] ?? $_POST['Person']?>">
                ...
                <div class="mb-3">
                    <label for="position">Position: </label><input type="text" class="form-control" name="Position" id="position" aria-describedby="emailHelp" value="<?=$data['position'] ?? $_POST['Position']?>">
